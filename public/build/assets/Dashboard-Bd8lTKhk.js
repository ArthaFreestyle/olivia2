...
import{r as o,_ as U,j as e,$ as B,Y as H}from"./app-D_BfqPvX.js";function q({logistik:u}){const[a,_]=o.useState(null),[j,M]=o.useState([]),[O,P]=o.useState(!1),[L,g]=o.useState(null),[r,w]=o.useState(null),[S,p]=o.useState(!1),[k,y]=o.useState(!1),d=o.useRef(null);o.useEffect(()=>{const s=document.createElement("link");s.rel="stylesheet",s.href="https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/leaflet.css",document.head.appendChild(s);const i=document.createElement("link");return i.rel="stylesheet",i.href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css",document.head.appendChild(i),()=>{document.head.removeChild(s),document.head.removeChild(i)}},[]),o.useEffect(()=>(typeof window<"u"&&U(()=>import("./leaflet-src-DGpue5TR.js").then(s=>s.l),__vite__mapDeps([0,1,2])).then(s=>{if(d.current=s.default,!a){const i=s.default.map("map",{zoomControl:!1}).setView([-6.2,106.816666],13);s.default.control.zoom({position:"bottomright"}).addTo(i),s.default.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',maxZoom:19}).addTo(i);const t=document.createElement("style");t.textContent=`
                        .custom-marker {
                            display: flex;
                            align-items: center;
                    ...
                    `,document.head.appendChild(t);const n=s.default.Control.extend({options:{position:"bottomright"},onAdd:function(){const l=s.default.DomUtil.create("div","locate-control"),m=s.default.DomUtil.create("div","locate-btn",l);return m.innerHTML='<i class="fas fa-location-crosshairs"></i>',s.default.DomEvent.on(m,"click",()=>$(i,s.default)),l}});new n().addTo(i),_(i),setTimeout(()=>i.invalidateSize(),100)}}),()=>{a&&a.remove()}),[]),o.useEffect(()=>{if(a&&d.current&&(u!=null&&u.routes)){P(!0),g(null);try{if(!u.routes||u.routes.length===0)throw new Error("Tidak ada rute yang ditemukan");console.log("Logistik Data:",u);const s=u.routes.map(i=>{const n=i.weight_filled||0,l=(n/20*100).toFixed(2),m=5e5,b=1-l/100*.3,c=(m*b).toFixed(0),h=(c*n).toFixed(0),f=i.geometry,x=d.current.geoJSON(f,{style:{color:"#3366FF",weight:6,opacity:.7,lineCap:"round",lineJoin:"round"}}).addTo(a),C=f.coordinates.map(z=>[z[1],z[0]]),v=d.current.polyline(C,{color:"#1e40af",weight:3,opacity:.9,dashArray:"10, 15",lineCap:"round",lineJoin:"round"}).addTo(a);let A=0;const E=()=>{A-=1,v.setStyle({dashOffset:A}),requestAnimationFrame(E)};E();const N=d.current.marker([i.points[0].latitude_start,i.points[0].longitude_start],{title:`Titik Awal: ${i.name}`,icon:d.current.divIcon({html:'<div class="custom-marker start-marker"><i class="fas fa-regular fa-truck"></i></div>',className:"",iconSize:[40,40],iconAnchor:[20,20]})}).addTo(a),T=d.current.marker([i.points[0].latitude_end,i.points[0].longitude_end],{title:`Nama Rute: ${i.name}`,icon:d.current.divIcon({html:'<div class="custom-marker end-marker"><i class="fas fa-flag-checkered"></i></div>',className:"",iconSize:[40,40],iconAnchor:[20,20]})}).addTo(a),R=K(N.options.title,i.driver||"Unknown Driver","Truk Kontainer",20,n,"PT Logistik Cepat","B 1234 KLM",l,c,h,i.distance,i.duration);return N.bindPopup(R,{maxWidth:300,className:"custom-popup"}),T.bindPopup(R,{maxWidth:300,className:"custom-popup"}),N.on("click",()=>{w({type:"start",lat:i.points[0].latitude_start,lng:i.points[0].longitude_start,title:`Titik Awal: ${i.name}`,info:{driver:i.driver||"Unknown Driver",vehicle:"Truk Kontainer",freightCapacity:20,freightFilled:n,company:"PT Logistik Cepat",licensePlate:"B 1234 KLM",freightPercentage:l,pricePerTon:c,totalPrice:h,distance:(i.distance/1e3).toFixed(2),duration:Math.round(i.duration/60)}}),p(!0)}),T.on("click",()=>{w({type:"end",lat:i.points[0].latitude_end,lng:i.points[0].longitude_end,title:`Titik Akhir: ${i.name}`,info:{driver:i.driver||"Unknown Driver",vehicle:"Truk Kontainer",freightCapacity:20,freightFilled:n,company:"PT Logistik Cepat",licensePlate:"B 1234 KLM",freightPercentage:l,pricePerTon:c,totalPrice:h,distance:(i.distance/1e3).toFixed(2),duration:Math.round(i.duration/60)}}),p(!0)}),{main:x,animated:v,startMarker:N,endMarker:T}});M(s),a.fitBounds(d.current.featureGroup(s.map(i=>i.main)).getBounds(),{padding:[50,50]})}catch(s){g(s.message||"Gagal memproses rute")}finally{P(!1)}}},[a,u]);const $=(s,i)=>{if(!s||!i)return;y(!0);const t=document.querySelector(".locate-btn");if(t){t.classList.add("locate-btn-active");const n=document.createElement("div");n.className="pulsing-circle",t.appendChild(n)}if("geolocation"in navigator)navigator.geolocation.getCurrentPosition(n=>{const{latitude:l,longitude:m}=n.coords,b=i.divIcon({html:'<div class="custom-marker" style="background-color: #2196F3;"><i class="fas fa-user"></i></div>',className:"",iconSize:[40,40],iconAnchor:[20,20]}),c=i.circle([l,m],{radius:n.coords.accuracy,color:"#2196F3",fillColor:"#2196F3",fillOpacity:.15,weight:2}).addTo(s);if(i.marker([l,m],{icon:b}).addTo(s).bindPopup('<div class="font-semibold">Lokasi Anda</div>',{className:"custom-popup"}),s.setView([l,m],16),y(!1),t){t.classList.remove("locate-btn-active");const f=t.querySelector(".pulsing-circle");f&&t.removeChild(f)}setTimeout(()=>{if(s.hasLayer(c)){const f=setInterval(()=>{const x=c.options.fillOpacity;x<=.02?(clearInterval(f),s.removeLayer(c)):c.setStyle({fillOpacity:x-.02,opacity:x-.02})},100)}},5e3)},n=>{if(console.error("Error getting location:",n),g("Gagal mendapatkan lokasi Anda. Periksa izin lokasi browser Anda."),y(!1),t){t.classList.remove("locate-btn-active");const l=t.querySelector(".pulsing-circle");l&&t.removeChild(l)}},{enableHighAccuracy:!0,timeout:1e4,maximumAge:0});else if(g("Browser Anda tidak mendukung geolocation"),y(!1),t){t.classList.remove("locate-btn-active");const n=t.querySelector(".pulsing-circle");n&&t.removeChild(n)}},K=(s,i,t,n,l,m,b,c,h,f,x,C)=>{const v=document.createElement("div");return v.className="driver-info-card",v.innerHTML=`
            <h3>${s}</h3>
            <div class="driver-info-item">
                <div class="driver-info-icon"><i class="fas fa-user"></i></div>
                <div>Driver: <strong>${i}</strong></div>
            </div>
            <div class="driver-info-item">
